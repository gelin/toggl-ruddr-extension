...
function TogglButton($$anchor, $$props) {
  ...
  var node = sibling(button, 2);
  {
    ...
    if_block(node, ($$render) => {
      if (get(panelVisible)) $$render(consequent);
    });
  }
...
function togglGetReportDate() {
  ...
  const dateInput = form == null ? void 0 : form.querySelector('input[name="date"]');
  return DateTime.fromFormat(dateInput == null ? void 0 : dateInput.value, "d/M/yyyy").toFormat("yyyy-MM-dd");
}
function togglFindForm() {
  const dialogDiv = document.querySelector("body > div:last-of-type");
  if (!dialogDiv) {
    return null;
  }
